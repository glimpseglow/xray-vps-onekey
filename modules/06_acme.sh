...
install_issue_cert() {
  ...
  local wildcard="${WILDCARD:-false}"
  local cert_domain acme_issue_domain

  # 证书目录和 acme.sh 的域名：
  #   --wildcard 时用主域名签泛域名证书，证书目录用主域名
  #   普通模式用 --domain 的值，证书目录也是 --domain 的值
  if [[ "$wildcard" == "true" ]]; then
    cert_domain="$(base_domain "$domain")"
    acme_issue_domain="$cert_domain"
  else
    cert_domain="$domain"
    acme_issue_domain="$domain"
  fi

  local cert_dir="/etc/nginx/ssl/${cert_domain}"
  local acme_cert_dir="/root/.acme.sh/${cert_domain}_ecc"
  local acme_sh="/root/.acme.sh/acme.sh"
  local acme_email="${ACME_EMAIL:-admin@${cert_domain}}"
  info "正在安装 acme.sh 并为 ${domain} 签发证书（证书目录: ${cert_dir}）..."
  ...
  if [[ -s "${acme_cert_dir}/fullchain.cer" && -s "${acme_cert_dir}/${cert_domain}.key" ]]; then
    if openssl x509 -in "${acme_cert_dir}/fullchain.cer" -noout 2>/dev/null; then
      already_issued="true"
      info "acme.sh 已有 ${cert_domain} 的有效证书，直接安装。"
    fi
  fi
  ...
  if [[ "$already_issued" == "true" && "${RENEW_CERT:-false}" != "true" ]]; then
    ...
    "$acme_sh" --install-cert -d "$cert_domain" --ecc \
    ...
    if [[ "${RENEW_CERT:-false}" == "true" ]]; then
      info "强制续签证书..."
      "$acme_sh" --renew -d "$cert_domain" --ecc --force --dnssleep 60 2>&1 || true
    else
      info "正在签发证书（DNS 验证，等待 60 秒）..."
      if [[ "$wildcard" == "true" ]]; then
        "$acme_sh" --issue --dns dns_cf -d "$cert_domain" -d "*.${cert_domain}" --dnssleep 60 \
          --key-file       "${cert_dir}/private.key" \
          --fullchain-file "${cert_dir}/fullchain.cer" 2>&1 || true
      else
        "$acme_sh" --issue --dns dns_cf -d "$cert_domain" --dnssleep 60 \
          --key-file       "${cert_dir}/private.key" \
          --fullchain-file "${cert_dir}/fullchain.cer" 2>&1 || true
      fi
    fi
  ...
  if validate_cert "${cert_dir}/fullchain.cer" "${cert_dir}/private.key"; then
    success "证书已安装: ${cert_domain}（目录: ${cert_dir}）。"
  else
...
可用命令排查:
  $acme_sh --list
  $acme_sh --issue --dns dns_cf -d $cert_domain --dnssleep 60 --debug 2
